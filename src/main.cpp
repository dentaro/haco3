      ...
      // if (commaIndex != -1) {
        ...
        // for (int i = 1; i < 1; i++) {
          ...
          if (nextCommaIndex != -1) {
            ...
            if(val.toInt() != NULL)isEditMode = val.toInt();
            else isEditMode = 0;//configファイルが壊れていても強制的に値を入れて立ち上げる
          }
        // }
      // }
    }
...
{
  // M5.begin();
  // M5.begin(false, false, false, false, kMBusModeInput, false);//外部給電のみ有効
  M5.begin(false, true, true, true, kMBusModeInput ,true);//SD,シリアルtrue
  // delay(500);
...
//psram使えない-------------------------------------------
  ...
  ui.setTouchZoom(1);//1倍設定
  tft.setPsram( true );//DMA利用のためPSRAMは切る
  tft.createSprite( 232, 234 );//PSRAMを使わないギリギリ
  tft.startWrite();//CSアサート開始

    // ui.setTouchZoom(1);//1倍設定
    // tft.setPsram( false );//DMA利用のためPSRAMは切る
    // tft.createSprite( 256, 208 );
    // tft.startWrite();//CSアサート開始

    // scaler.setPsram( false );//DMA利用のためPSRAMは切る
    // scaler.createSprite( 256, 32 );
    // scaler.startWrite();//CSアサート開始

    //-----------コーディングモード関連

    editor.initEditor(tft);  
    // String gamepath = appfileName ;
    // String gamepath = "/star2/main.lua";
...
void loop()
{
  ...
  if(isEditMode==0){
    ...
    if(Prs.getKeycode()==27){//キーボードからエディタ再起動
  ...
  }else if(isEditMode == 1){//コーディングモードの時

    // Serial.print("KEYCODE:");
    // Serial.println(Prs.getKeycode());


    if(ui.getTouchBtnID() == 8||Prs.getKeycode()==27){
      // isEditMode = 0;//コーディングモード終了して再起動
      editor.editorSave(SPIFFS);
      delay(100);//ちょっと待つ
      editor.editorSaveSD(SD);//SDに同時にバックアップ保存する
      delay(100);//ちょっと待つ
      setOpenConfig(appfileName, 0);
      reboot();
    }

    if(ui.getTouchBtnID() == 0){
      // isEditMode = 0;//コーディングモード終了して再起動
    ...
    // // char c = Usb.getChar();
    // // char buffer[64];
    // // Usb.getStr(buffer, sizeof(buffer));

    
    //   // 'あ'の場合の処理
    //   Serial.println(c + "が入力されました。");


    // Usb.Task();
    if(ui.getEvent() == TOUCH||ui.getEvent() == MOVE){
      if(ui.getTouchBtnID() == 1){editor.editorMoveCursor(ARROW_LEFT);}
      if(ui.getTouchBtnID() == 2){editor.editorMoveCursor(ARROW_RIGHT);}
      if(ui.getTouchBtnID() == 3){editor.editorMoveCursor(ARROW_UP);}
      if(ui.getTouchBtnID() == 4){editor.editorMoveCursor(ARROW_DOWN);}
    }

    editor.update(tft, SPIFFS, SD, Prs);



    // nowTime = millis();
    // editor.editorRefreshScreen(tft);
    // if(editor.getTargetTime() == 0){
    //   editor.editorProcessKeypress(Prs.getKeyNo(), SPIFFS);//キーコードとセーブ先メディアを渡す　初めに一回押される
    //   editor.setTargetTime(nowTime + waitTime);
    // }
    
    // if(editor.getTargetTime() == -1 || nowTime <= editor.getTargetTime()){//waitTime中は
    //   //何もしない
    // }

    // if(nowTime>editor.getTargetTime()){
    //   editor.editorProcessKeypress(Prs.getKeyNo(),SPIFFS);//キーアップするまで続く
    // }
    ...
    // if(targetTime == 0){
    //   editor.editorProcessKeypress(Prs.getKeyNo(), SPIFFS);//キーコードとセーブ先メディアを渡す　初めに一回押される
    //   targetTime = nowTime + waitTime;
    // }
    
    // if(targetTime == -1 || nowTime <= targetTime){//waitTime中は
    //   //何もしない
    // }
    ...
    //   editor.editorProcessKeypress(Prs.getKeyNo(),SPIFFS);//キーアップするまで続く
    // }
    ui.showTouchEventInfo( tft, 240, 0);//タッチイベントを視覚化する
    ui.showInfo( tft, 110, 0 );//ボタン情報、フレームレート情報などを表示します。
    
    tft.pushSprite(&screen,24,0);
  ...
  // if(btn(0) == true)isEditMode == false;
    
    
    // scaler.fillScreen(HACO3_C0);
    // scaler.setTextSize(3);
    // scaler.setTextColor( TFT_WHITE , TFT_BLACK );
    // scaler.setFont(&lgfxJapanGothicP_8);//日本語可
    // scaler.setCursor(0, 1);//位置
    // scaler.print(" Sample拡大鏡");

    // scaler.setTextSize(1);

    // ui.showTouchEventInfo( tft, 120, 0);//タッチイベントを視覚化する
    // ui.showInfo( tft, 120, 8 );//ボタン情報、フレームレート情報などを表示します。

    // scaler.pushSprite(&screen,0, 208);//コード拡大鏡
